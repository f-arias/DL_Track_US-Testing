...
| `hausdorff_distance()`   | Distancia de Hausdorff                | Mide el error entre contornos. Soporta `scipy` y `scikit-image` (con método 'standard' o 'modified'). |
...
# --- Cálculo de métricas de superposición ---
...
# --- Cálculo de Distancia de Hausdorff con diferentes opciones ---
# Usando la implementación de SciPy (por defecto)
hd_scipy = US_metrics.hausdorff_distance(mascara_referencia, mascara_prueba, library='scipy')

# Usando la implementación de Scikit-image con el método modificado (más robusto a outliers)
hd_skimage_modified = US_metrics.hausdorff_distance(
    mascara_referencia,
    mascara_prueba,
...
print(f"Distancia de Hausdorff (SciPy): {hd_scipy:.4f}")
print(f"Distancia de Hausdorff Modificada (Scikit-image): {hd_skimage_modified:.4f}")
```
